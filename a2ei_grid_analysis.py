...
df = data_prep(df)
# df_grid
df_grid = pd.DataFrame(columns=['input_voltage', 'output_voltage', 'input_voltage_0tonan',
                       'output_voltage_5tonan', 'grid_avl', 'grid_on_time', 'grid_off_time', 'aam_on_time', 'aam_off_time'])
...
# drop nan rows in df_grid
df_grid.dropna(subset=['input_voltage'], inplace=True)
...
# inplace overwrite the original data frame
df_grid['grid_on_time'].mask(df_grid['grid_avl'] > 0, 5, inplace=True)
...
# makes the 1 to 0 transtion back to 0 just to count BL
df_grid['counter_bl'] = df_grid['counter_bl'].replace([1], 0)
# hour_data
...
daily_data['grid_on_time_daily'] = df_grid['grid_on_time'].resample(
    'D').sum()/60
# make all nan to 0, just consider the days with >0 grid connection time
daily_data['grid_on_time_daily'].replace(0, np.nan, inplace=True)
daily_data['grid_off_time_daily'] = df_grid['grid_off_time'].resample(
    'D').sum()/60
daily_data['aam_on_time_daily'] = df_grid['aam_on_time'].resample('D').sum()/60
daily_data['aam_off_time_daily'] = df_grid['aam_off_time'].resample(
    'D').sum()/60
...
df_grid_evt = df_grid.groupby('counter_evt')['grid_avl'].min().to_frame(
    name='grid_avl').join(df_grid.groupby('counter_evt')['grid_avl'].count().rename('length'))
# lenght blackuts in hours #.mask(df_grid_evt['grid_avl']!=0)
df_grid_evt['length_h'] = df_grid_evt['length']*5/60
# drop first and last row, normally this events are not complete
df_grid_evt.drop(df_grid_evt.tail(2).index, inplace=True)
df_grid_evt.drop(df_grid_evt.head(2).index, inplace=True)
# get just on grid off events
df_grid_evt_off = pd.DataFrame(columns=[])
df_grid_evt_off = df_grid_evt[df_grid_evt['grid_avl'] == 0].sort_values(
    by='length_h')  # , ascending = False)
...
grid_on_time_all = round(
    (daily_data['grid_on_time_daily'].sum(skipna=True)), 1)
...
with st.expander('Grid Analysis'):
    ...
    st.markdown("<h3 style='text-align: center'>Grid and AAM Availability</h3>",
                unsafe_allow_html=True)
    ...
    with col1:
        ...
        fig.update_layout(annotations=[dict(
            text="Grid Availability", x=0.5, y=0.5, font_size=12, showarrow=False)])
    ...
    with col2:
        ...
        fig.update_traces(hoverinfo='label+percent', textinfo='label+percent', textfont_size=12,
                          marker=dict(colors=colors, line=dict(color='#000000', width=2)))
        fig.update_layout(annotations=[dict(
            text="AAM Availability <br> All Day", x=0.5, y=0.5, font_size=12, showarrow=False)])
        fig.update_layout(font=dict(family="Fugue", size=12, color='black'),
                          autosize=False,
                          width=300, height=300,
                          margin=dict(l=15, r=15, b=15, t=15,  pad=2),
...
################################
    ...
    st.markdown("<h3 style='text-align: center'>Power Outage Duration</h3>",
                unsafe_allow_html=True)
    ...
    with col3:
        ...
        fig.add_trace(go.Histogram(x=df_grid_evt_off['length_h'], histnorm='percent',
                      marker_color='rgb(223,116,149)', cumulative_enabled=False, name='Histogram'))
        fig.add_trace(go.Histogram(x=df_grid_evt_off['length_h'], histnorm='percent',
                      marker_color='lightslategrey', cumulative_enabled=True, name='Cumulative Histogram'))
        #name='Blackouts per day',
    ...
    ##################################################################################
    st.markdown("<h3 style='text-align: center'>Power Outage Frequency</h3>",
                unsafe_allow_html=True)
