...
st.set_page_config(
    page_title="A2EI Grid Analysis Tool",
    page_icon=":sunny:",
...
st.sidebar.header('Metadata AAM : '+aam_id)
...
df_grid['peak_voltage_0tonan'].mask(df_grid['peak_voltage']>0,df_grid['peak_voltage'],inplace=True) #takes out the 0V data points
...
with st.expander('Data Overview for AAM '+aam_id):
    ...
    final.add_trace(go.Scatter(x = df.index, y = df['input_voltage'], name =  'input_voltage',  line_color = 'rgba(82, 82, 82, .8)'))         
    final.add_trace(go.Scatter(x = df.index, y = df['peak_voltage'], name = 'peak_volatge',  line_color = 'rgb(245, 237, 111)'))
    final.add_trace(go.Scatter(x = df.index, y = df['output_voltage'], name =  'output_voltage', line_color = 'rgb(251, 117, 157)'))
